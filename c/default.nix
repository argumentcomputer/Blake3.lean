...
let
  ...
  blake3-c = import ./lib.nix {
    inherit pkgs system blake3;
  };
    ...
    ({ archive ? false
     ...
     , debug ? false
      ...
      let
        ...
        commonCCOptions = [ "-Wall" "-O3" "-fPIC" "-I${lean-bin-tools-unwrapped}/include" "-I${blake3}/c" ]
          ++ (if debug then [ "-ggdb" "-DDEBUG" ] else [ ]) ++ ccOptions;
