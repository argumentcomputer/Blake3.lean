...
def finalizeBlake3Hash (hasher : Hasher) : Blake3Hash :=
  let ⟨hash, h⟩ := hasher.finalize BLAKE3_OUT_LEN.toUSize
  have hash_size_eq_BLAKE3_OUT_LEN : hash.size = BLAKE3_OUT_LEN := by
    cases System.Platform.numBits_eq with
    | inl h32 => simp [h, h32]; norm_cast
    | inr h64 => simp [h, h64]; norm_cast
  ⟨hash, hash_size_eq_BLAKE3_OUT_LEN⟩

def finalizeBlake3Key (hasher : Hasher) : Blake3Key :=
  let ⟨hash, h⟩ := hasher.finalize BLAKE3_KEY_LEN.toUSize
  have hash_size_eq_BLAKE3_OUT_LEN : hash.size = BLAKE3_KEY_LEN := by
    cases System.Platform.numBits_eq with
    | inl h32 => simp [h, h32]; norm_cast
    | inr h64 => simp [h, h64]; norm_cast
  ⟨hash, hash_size_eq_BLAKE3_OUT_LEN⟩
...
def ratchet (sponge : Sponge) : Sponge :=
  let key := sponge.hasher.finalizeBlake3Key
